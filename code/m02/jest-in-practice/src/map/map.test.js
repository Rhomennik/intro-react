...
it('map[1, 2] should return return [1, 2]', () => {
...
it('map(1) should throw a TypeError with message "THe first parameter should must be an array"', () => {
    let error
    try {
        map(1)
    } catch (e) {
    ...
    expect(error.message).to.be.equal('The first parameter should must be an array')
})
it('map(1) should throw a TypeError with message "The first parameter should must be an array"', () => {
    let error
    try {
